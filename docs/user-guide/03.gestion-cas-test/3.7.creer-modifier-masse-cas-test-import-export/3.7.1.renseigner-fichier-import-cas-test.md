...
Le fichier zip doit contenir un tableur excel avec un onglet (le nom de l’onglet correspond au nom du cas de test à importer), les autres seront ignorés.
Pour importer plusieurs cas de test, il faut créer plusieurs fichiers (1 fichier = 1 onglet = 1 cas de test).
Le tableur doit contenir les balises suivantes dans le même ordre que ci-dessous :
